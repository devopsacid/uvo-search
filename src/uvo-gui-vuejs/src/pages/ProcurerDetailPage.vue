...
<template>
  ...
  <div v-else-if="summary" class="flex flex-col gap-3">
    <div class="flex items-baseline justify-between gap-3">
      <div>
        <p class="text-xs muted uppercase tracking-wider">{{ t('procurers.title') }} · IČO <span class="mono">{{ summary.ico }}</span></p>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <StatCard :label="t('procurers.totalSpend')" :value="fmtValue(summary.total_spend ?? summary.total_value ?? 0)" />
    ...
    <Panel :title="t('entities.topSuppliers')" dense>
      <div v-if="(detail?.top_suppliers as TopSup[] | undefined)?.length" class="flex flex-col">
        <div
        ...
        >
          ...
          <span class="text-right font-semibold num">{{ fmtValue(s.total_value) }}</span>
