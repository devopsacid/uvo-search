...
import { useProcurerPeriodSummary } from '@/api/queries/analytics'
...
export function ProcurerAnalyticsPage() {
  const { ico } = useParams<{ ico: string }>()
  const safeIco = ico ?? ''
  const { dateFrom, dateTo } = useDateRange()

  const { data, isLoading, isError } = useProcurerPeriodSummary(safeIco, dateFrom, dateTo)

  const searchLink = `/search?procurer_ico=${safeIco}&date_from=${dateFrom}&date_to=${dateTo}`

  if (isError) {
    return (
      <div className="p-6 text-center">
        <p className="text-muted-foreground">{sk.analytics.common.error}</p>
        <Link to={`/procurers/${safeIco}`} className="mt-4 block text-sm text-primary hover:underline">
          ← {sk.analytics.procurer.breadcrumb}
        </Link>
      </div>
    )
  }
    ...
    <div className="space-y-6 print:space-y-4">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <Link to={`/procurers/${safeIco}`} className="hover:underline">
              {sk.analytics.procurer.breadcrumb}
            </Link>
            <span>/</span>
            <span>{sk.analytics.procurer.title}</span>
          </div>
          {isLoading ? (
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {isLoading ? (
          ...
          <>
            <KpiCard
              label={sk.analytics.procurer.kpiSpend}
            ...
            />
            <KpiCard
              label={sk.analytics.procurer.kpiSuppliers}
          ...
          <Table>
            ...
            <TableBody>
              {isLoading
                ...
                : (data?.top_counterparties ?? []).map((s, i) => (
                    <TableRow key={s.ico ?? i}>
                      <TableCell>
                        {s.ico ? (
                      ...
                      <TableCell className="text-right tabular-nums">
                        {formatNumber(s.contract_count)}
                      </TableCell>
                      <TableCell className="text-right tabular-nums">
                        {formatCurrency(s.total_value)}
                      </TableCell>
                      <TableCell>
                        <ShareBar pct={s.share_pct} />
                      </TableCell>
                    </TableRow>
                  ))}
            </TableBody>
