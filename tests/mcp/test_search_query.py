...
def test_quoted_phrase_uses_phrase_operator():
    ...
    assert stage == {"phrase": {"query": "Fakulta elektrotechniky", "path": ["name"]}}


def test_wildcard_star_uses_wildcard_operator():
    stage = build_search_stage("fakul*", ["name"])
    assert stage == {"wildcard": {"query": "fakul*", "path": ["name"], "allowAnalyzedField": True}}
...
def test_plain_query_uses_compound_autocomplete_plus_text():
    ...
    assert {
        "autocomplete": {"query": "fakulta", "path": "name", "fuzzy": {"maxEdits": 1}}
    } in should
    assert {"text": {"query": "fakulta", "path": ["name"], "fuzzy": {"maxEdits": 2}}} in should
